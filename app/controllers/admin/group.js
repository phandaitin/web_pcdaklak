...
const myModel= require(__path_models + '/group');
const userModel= require(__path_models + '/user');
...
let pageTitleList='Quản lý Group';
...
router.get('/', async (req,res,next) => {     
    try { 
        ...
        // await myModel.countDocuments({objWhere}).then( (zdata) => {
        //     pagination.totalRows = zdata;
        //  });
...
router.post( ('/save'), async (req,res,next) => {     // xử lý cho cả 2 TH add và edit
    try {          
        ...
        if(typeof data !== 'undefined' && data.id !==''){// EDIT                
            ...
            await userModel.updateMany({'tmain.id': data.id} , {
                tmain : {
                    id	    :   data.id ,
                    name	:   data.name 
                }
            ...
            if(group){
                res.render(`${folderView}form`,{pageTitle: pageTitleAdd ,data: {err: 'Group Exit'} } )
            }else{
