...
router.post( ('/save'), async (req,res,next) => {     // xử lý cho cả 2 TH add và edit
    try {          
        ...
        if(typeof data !== 'undefined' && data.id !==''){// EDIT                
            ...
            console.log(data);                        
            await baivietModel.updateMany({'tmain.id': data.id} , {
            tmain : {
                    id	   :  data.id ,
                    name	: data.name,
                }               
            ...
            let chuyenmuc = await myModel.findOne( { name: req.body.name})                
            if(chuyenmuc){
                res.render(`${folderView}form`,{pageTitle: pageTitleAdd ,data: {err: 'chuyenmuc Exit'} } )
            }else{
