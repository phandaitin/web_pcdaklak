...
const userValidates  = require(__path_validates + '/user')
let collection ='user'
...
let pageTitleList= 'Quản lý User' ;
...
router.get(('/form(/:id)?'),  async (req, res, next) => {
    ...
    let data    = {} ;
    let notify  = {};
    let dataGroup =  await groupModel.find({},{ _id : 1 , name : 1 });
        dataGroup.unshift({_id: 'Novalue', name: 'Chọn group'});
    try {      
        if( id === '')  {  //add
...
router.post( ('/save'),  uploadsHelpers.uploadFile('fileUpload', collection), async (req,res,next) => {      
    ...
    let dataGroup =  await groupModel.find({},{ _id : 1 , name : 1 });
            dataGroup.unshift({_id: 'Novalue', name: 'Chọn groups'});
    // VALIDATES
     if(req.body.group_id =='Novalue' || req.body.name=='' ){
        ...
        try {
            if(typeof data !== 'undefined' && data !== undefined && data.id !==''){// EDIT  data.id laf type hidden          
                ...
                //////////////////////////////////////////////////
                data.tmain ={    //tmain là collection của bảng USER
                    id 		: data.group_id,
                ...
                let checkExistsName = await myModel.findOne( { name: req.body.name})    // neeus user ton tai            
                if(checkExistsName){                 
                    res.render(`${folderView}form`,{
                        pageTitle: pageTitleAdd ,
